...
def repetition_score(tokens, n=3) -> int:
    ngrams: list[tuple[Any, ...]] = [
        tuple[Any, ...](tokens[i : i + n]) for i in range(len(tokens) - n + 1)
    ]
...
def base_hallucination(src, tgt):
...
def detect_hallucination(src: str, tgt: str, src_lang: str):
